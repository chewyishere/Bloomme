...
export default class FlowerRoot {
  constructor(scene, data) {
    this.scene = scene;
    this.root = null;
    this.rootLength = data.rootLength;
    this.leafNum = data.leafNum;
    this.rootGroup = null;
    this.rootWave = 1; //todo: load from flower randomizer obj
    this.leaf = null;
  }
  ...
  init() {
    ...
    this.leaf.init();
    this.makeRoot(this.leafNum, this.rootLength, this.rootWave);
  }
  ...
  makeRoot(leafNum, rootLength, rootWave) {
    ...
    var curve = new THREE.CatmullRomCurve3(
      this.curvePoints(rootLength, rootWave)
    );
